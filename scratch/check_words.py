...
# Let's perform a smart condensation to reach ~2,100 words main body count
# We keep every section, every case, every quote, both race conditions, exact death count, etc.
...
In the history of medical technology, few failures carry the systemic weight of the Therac-25 radiation therapy accidents. Between June 1985 and January 1987, six patients in clinics across the United States and Canada received massive radiation overdoses during routine treatments for cancer. The machine, a dual-mode medical linear accelerator designed to destroy malignant tumors, delivered invisible beams of energy up to 250 times the prescribed dose.

These overexposures caused severe tissue necrosis, loss of limb function, internal organ destruction, and three direct patient deaths. Unlike traditional industrial accidents caused by exploding pressure vessels or structural steel fractures, the catastrophes of the Therac-25 occurred quietly inside the silicon logic of a PDP-11 minicomputer. The machine appeared to function normally from the operator's console, even as it delivered lethal radiation into living tissue.

The Therac-25 has since become a classic case study in software engineering, safety-critical system design, and human-computer interaction. It demonstrated what happens when physical safety mechanisms are replaced entirely by software code, when legacy subroutines are reused without re-testing in new hardware environments, and when corporate developers treat software defects as physically impossible events.
...
with open(r"d:\MyWeb\Night Lore\scratch\condense_script.py", "w", encoding="utf-8") as f:
    f.write('''
import re

path = r"d:\\MyWeb\\Night Lore\\src\\content\\stories\\therac-25-radiation-accidents.md"
with open(path, "r", encoding="utf-8") as f:
    text = f.read()

parts = text.split("## Verification Checklist")
body_and_front = parts[0]
checklist = parts[1]

# Check word count of main narrative:
body_narrative = body_and_front.split("## The Blind Machine")[1]
words = len(("## The Blind Machine" + body_narrative).split())
print("Current Main Body Word Count:", words)
''')
